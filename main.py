...
def get_env_metadata(env, brain_name):
    ...
    return state_size, action_size, len(env_info.agents)
...
def ddpg(env, brain_name, agent, config, train_mode=True, weights_path=None):
    ...
    for i_episode in range(1, config['train']['n_episodes'] + 1):
        ...
        for t in range(config['train']['max_t']):
            ...
            if np.any(done):
                break
        scores_rolling.append(np.mean(score))  # save most recent score
        scores.append(np.mean(score))  # save most recent score

        # print('\nEpisode {}\tAverage Score: {:.2f}'.format(i_episode, np.mean(scores_rolling)))
        if train_mode:
            if i_episode % 10 == 0:
                print('Episode {}\tAverage Score: {:.2f}'.format(i_episode, np.mean(scores_rolling)))
...
def main(config_path, train_mode=True, weights_path=None):
    ...
    state_size, action_size, num_agents = get_env_metadata(env, brain_name)
...
if __name__ == "__main__":
    ENV_FILE = "Tennis.app"
    parser = argparse.ArgumentParser(description='Tennis Agent')
